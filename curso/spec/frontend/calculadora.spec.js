...
describe.skip('Pruebas de la calculadora', () => {
	...
	describe('Controladores de eventos', () => {
		it('Controladores: por constructor', () => {
			new Calculadora(fnPantalla, fnResumen)
			expect(pantallaHasCalled).toBeTruthy()
			expect(pantalla).toBe('0')
			expect(resumenHasCalled).toBeTruthy()
			expect(resumen).toBe('')
		});

		it('Invalidas', () => {
			expect(() => new Calculadora('mala')).toThrow()
			expect(() => new Calculadora(nop, 'no es una función')).toThrow()
		});
	});
